...
template <uint8_t PIN_1, uint8_t PIN_2_3, uint8_t PIN_4>
class Vid28Stepper {
public:
  ...
  void init() {
    ...
    for (auto i = 1; i < 32; ++i) {
      detail::accel_table_[i] = detail::accel_ * static_cast<float>(detail::accel_table_[i-1]);
    }
  ...
  void set_zero(float degrees) {
    zero_angle_ = std::round(degrees * 12);
  }
  ...
  void step_to(float degrees, float hz) {
    ...
    std::cout << "step_to went to angle = " << current_angle_/12.0 << std::endl;
  }

private:
  auto step_micros(uint microstep_count, float hz, bool forward) {
    uint64_t us_per_microstep = 1.0 / (hz*1.0E-6 * 360 * 12);
    std::cout << "I will wait " << us_per_microstep << " us between steps" << std::endl;
    uint16_t wait_us;
    for (auto s = 0; s < microstep_count; ++s) {
      if (s < 2 * detail::accel_rampup_ * 32) {
        ...
        wait_us = detail::accel_table_[31 - (microstep_count - s) / detail::accel_rampup_];
